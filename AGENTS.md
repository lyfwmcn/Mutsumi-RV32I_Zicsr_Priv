...
- The RTL was renamed to snake_case and all state initializes via `initial` blocks (`src/` has **no reset port**). Both filelists and both bus variants were ported; `make`, `make all`, and `make sim` work end-to-end (verified on `Develop`).
- `tools/stage_timing.py` still keys on CamelCase stage tokens (`IFStage`, `IDStage`, ...), so `make timing` runs but lumps every net into `OTHER`. Update those tokens to the snake_case stage names before trusting its per-stage table.
...
- `sim/` — simulation only: `system_bus.v` (behavioral 4 KB byte memory, `$fread("build/sim_test.bin")`, `$write`s a byte on a store to **0xFFC**) and `tb.v`.
- `syn/` — board: `top.v` (wires `cpu` + `system_bus`), `top.lpf`, `system_bus.v` (true-dual-port `DP16KD` EBR lanes + UART), `uart_tx.v`. `sim/system_bus.v` and `syn/system_bus.v` now share the module name and port list but differ internally (behavioral vs EBR); `tb.v` instantiates the sim one, `top.v` the syn one. A port change still needs both.
